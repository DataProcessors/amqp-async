...
class AMQPChannel
{
      ...
      foreach ($looking_for_methods as $looking_for_method) {
        if ($method_sig == $this->waitHelper->get_wait($looking_for_method)) {
          ...
          if ($looking_for_method == 'queue.declare_ok') {
            ...
            //do nothing for: exchange.declare_ok, exchange.delete_ok, channel.flow_ok, queue.bind_ok, queue.unbind_ok, queue.delete_ok, queue.purge_ok, tx.select_ok, tx.commit_ok, tx.rollback_ok, basic.qos_ok
            //
   ...
   */
  public function basic_return($payload) {
    $args = substr($payload, 4, strlen($payload) - 4);
    $this->bufferReader->reuse($args);
    $msg = new AMQPMessage();
