...
class AMQPBufferWriter extends AbstractClient
{
    ...
    {
        if (($bytes <= 0) || ($bytes % 2)) {
            throw new Exception\AMQPInvalidArgumentException(sprintf('Expected bytes count must be multiply of 2, %s given', $bytes));
        }
        ...
        if (is_int($x)) {
            ...
            if (!is_numeric($x)) {
                throw new Exception\AMQPInvalidArgumentException(sprintf('Unknown numeric string format: %s', $x));
            }
            $x = preg_replace('/^-/', '', $x, 1, $isNeg);
        } else {
            throw new Exception\AMQPInvalidArgumentException('Only integer and numeric string values are supported');
        }
        if ($isNeg) {
            $x = bcadd($x, -1, 0);
        ...
        if ($x || ($isNeg && ($chnk & 0x8000))) {
            throw new Exception\AMQPOutOfBoundsException(sprintf('Overflow detected while attempting to pack %s into %s bytes', $ox, $bytes));
        }
    ...
    {
        if ($n < 0 || $n > 255) {
            throw new Exception\AMQPInvalidArgumentException('Octet out of range: ' . $n);
        }
    ...
    {
        if (($n < -128) || ($n > 127)) {
            throw new Exception\AMQPInvalidArgumentException('Signed octet out of range: ' . $n);
        }
    ...
    {
        if ($n < 0 || $n > 65535) {
            throw new Exception\AMQPInvalidArgumentException('Short out of range: ' . $n);
        }
    ...
    {
        if (($n < -32768) || ($n > 32767)) {
            throw new Exception\AMQPInvalidArgumentException('Signed short out of range: ' . $n);
        }
    ...
    {
        if (($n < 0) || ($n > 4294967295)) {
            throw new Exception\AMQPInvalidArgumentException('Long out of range: ' . $n);
        }
    ...
    private function write_signed_long($n)
    {
        if (($n < -2147483648) || ($n > 2147483647)) {
            throw new Exception\AMQPInvalidArgumentException('Signed long out of range: ' . $n);
        }
    ...
    {
        if ($n < 0) {
            throw new Exception\AMQPInvalidArgumentException('Longlong out of range: ' . $n);
        }
        ...
        if (bcadd($n, -PHP_INT_MAX, 0) <= 0) {
            ...
            } catch (Exception\AMQPOutOfBoundsException $ex) {
                throw new Exception\AMQPInvalidArgumentException('Longlong out of range: ' . $n, 0, $ex);
            }
        }
    ...
    public function write_signed_longlong($n)
    {
        if ((bcadd($n, PHP_INT_MAX, 0) >= -1) && (bcadd($n, -PHP_INT_MAX, 0) <= 0)) {
            ...
            throw new Exception\AMQPInvalidArgumentException('Signed longlong out of range: ' . $n);
        } else {
            ...
            try {
                ...
                throw new Exception\AMQPInvalidArgumentException('Signed longlong out of range: ' . $n, 0, $ex);
            }
        }
    ...
    public function write_shortstr($s)
    {
        $len = strlen($s);
        if ($len > 255) {
            throw new Exception\AMQPInvalidArgumentException('String too long');
        }
    ...
    private function write_value($type, $val)
    {
        ...
        switch ($type) {
            ...
            case AMQPAbstractCollection::T_VOID:
                break;
            default:
                throw new Exception\AMQPInvalidArgumentException(sprintf(
                    'Unsupported type "%s"',
                    $type
                ));
        }
