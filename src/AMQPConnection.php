...
class AMQPConnection {
  ...
  */
  public function connect(string $host, string $port, string $user, string $password, string $vhost = '/', string $login_method = 'AMQPLAIN', string $locale = 'en_US', int $heartbeat = 0) {
    ...
    ///
    $pumpCoroutine = new Coroutine($this->pump()); // pump needs to stay running as a coroutine, reading and dispatching messages
  }
  ...
  */
  ...
  public function setRateLimit($rate) {
    ...
    if ($frame_type == 3) { // FRAME-BODY
      if (!$ch->pendingClose) {
        ...
        if ($this->rateLimiter > 0) {
          yield Promise\resolve()->delay($this->rateLimiter);
        }
      }
    }
    else
